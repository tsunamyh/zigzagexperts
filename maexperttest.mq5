...
double Z_data[10];
int    Z_candles[10];
double MA_data_high[250];   // array for iMA indicator values
...
void OnTick() {
    ...
    if(bNewBarEvent) {
        ...
        CopyBuffer(MA_handle_High, 0, 0, 250, MABuf_High);
        CopyBuffer(MA_handle_Low, 0, 0, 250, MABuf_Low);

        int num     = 0;
        int counter = 0;
        for(int i = 249; i > 245; i--) {
            MA_data_high[249 - i] = MABuf_High[i];
            Print("MA_data_high[", 249 - i, "]", MA_data_high[249 - i]);
            if(ZigzagBuf[i] != 0.0) {
                if(num < 10) {
                    ...
                    if(num == 1) {
                        for(int j = 0; j < 11; j++) {
                            MA_data_low[j] = MABuf_Low[i - 5 + j];
                            // Print("MA_data_low[", j, "]", MA_data_high[j]);
                        }
                    }
        ...
        //     Print("MA_data_high[", i, "]: ", MA_data_high[i]);
        // }
        if(checkBuy()) {
            Print("Sharte Buy");
